...
class CapsNet3D(nn.Module):
    def __init__(self, in_ch=1, out_ch=1, xpad=True):
        ...
        self.PrimaryCaps2 = ConvCaps(
            Ci=1, Pi=16, Co=2, Po=16, K=5, stride=2, padding=2, routings=1
        )
        self.ConvCaps3 = ConvCaps(
            Ci=2, Pi=16, Co=4, Po=16, K=5, stride=1, padding=2, routings=3
        )
        self.ConvCaps4 = ConvCaps(
            Ci=4, Pi=16, Co=4, Po=32, K=5, stride=2, padding=2, routings=3
        )
        self.ConvCaps5 = ConvCaps(
            Ci=4, Pi=32, Co=8, Po=32, K=5, stride=1, padding=2, routings=3
        )
        self.ConvCaps6 = ConvCaps(
            Ci=8, Pi=32, Co=8, Po=64, K=5, stride=2, padding=2, routings=3
        )
        self.ConvCaps7 = ConvCaps(
            Ci=8, Pi=64, Co=8, Po=32, K=5, stride=1, padding=2, routings=3
        )
        self.DeconvCaps8 = DeconvCaps(
            Ci=8, Pi=32, Co=8, Po=32, K=4, stride=2, routings=3
        )
        self.ConvCaps9 = ConvCaps(
            Ci=16, Pi=32, Co=8, Po=32, K=5, stride=1, padding=2, routings=3
        )
        self.DeconvCaps10 = DeconvCaps(
            Ci=8, Pi=32, Co=4, Po=16, K=4, stride=2, routings=3
        )
        self.ConvCaps11 = ConvCaps(
            Ci=8, Pi=16, Co=4, Po=16, K=5, stride=1, padding=2, routings=3
        )
        self.DeconvCaps12 = DeconvCaps(
            Ci=4, Pi=16, Co=2, Po=16, K=4, stride=2, routings=3
        )
        self.FinalCaps13 = ConvCaps(
            Ci=3, Pi=16, Co=1, Po=16, K=1, stride=1, padding=0, routings=3
        )
...
class Conv(nn.Module):
    ...
    def __init__(self, in_ch, Po, K, stride, padding=None):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_ch, Po, K, stride, padding), nn.ReLU(inplace=True)
        )
...
class ConvCaps(nn.Module):
    ...
    def __init__(self, Ci, Pi, Co, Po, K, stride, padding, routings=3):
        ...
        self.conv = nn.Conv3d(
            Pi, Co * Po, kernel_size=K, stride=stride, padding=padding, bias=False
        )
        self.biases = nn.Parameter(
            torch.zeros(1, 1, Co, Po, 1, 1, 1) + 0.1
    ...
    def forward(self, x):  # x: [B, Ci, Pi, Di, Hi, Wi]
        ...
        x = x.reshape(
            B, Ci, self.Co, self.Po, Do, Ho, Wo
        )  # x: [B, Ci, Co, Po, Do, Ho, Wo]
        return dynamic_routing(
            x, self.biases, self.routings
        )  # return: [B, Co, Po, Do, Ho, Wo]
...
class DeconvCaps(nn.Module):
    ...
    def __init__(self, Ci, Pi, Co, Po, K, stride, routings=3):
        ...
        self.conv = nn.ConvTranspose3d(
            Pi, Co * Po, kernel_size=K, stride=stride, bias=False
        )
        self.biases = nn.Parameter(
            torch.zeros(1, 1, Co, Po, 1, 1, 1) + 0.1
        )  # biases: [1, 1, Co, Po, 1, 1, 1]
...
def dynamic_routing(votes, biases, routings):
    ...
    bij = torch.zeros(B, Ci, Co, 1, Do, Ho, Wo).to(
        device
    )  # bij: [B, Ci, Co, 1, Do, Ho, Wo]

    for t in range(routings):
        cij = F.softmax(bij, dim=2)  # cij: [B, Ci, Co, 1, Do, Ho, Wo]
        sj = (
            torch.sum(cij * votes, dim=1, keepdim=True) + biases
        )  # sj: [B, 1, Co, Po, Do, Ho, Wo]
        vj = squash(sj)  # vj: [B, 1, Co, Po, Do, Ho, Wo]
        if t < routings - 1:
            bij = bij + torch.sum(
                votes * vj, dim=3, keepdim=True
            )  # bij: [B, Ci, Co, 1, Do, Ho, Wo]
...
def squash(sj):
    ...
    """
    sjnorm = torch.linalg.norm(
        sj, dim=3, keepdim=True
    )  # sjnorm: [B, 1, Co, 1, Do, Ho, Wo]
    sjnorm2 = sjnorm**2
    return (
        sjnorm2 * sj / ((1 + sjnorm2) * sjnorm)
    )  # return vj: [B, 1, Co, Po, Do, Ho, Wo]
